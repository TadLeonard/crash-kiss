...
def get_ascii_edges(img, char=u"@"):
    return "\n".join(_gen_ascii_edges(img, char))


def _gen_ascii_edges(img, char):
    for row in img:
        left, right, _ = get_edge_indices(row)
        yield (u" " * left) + (char * (right - left))
...
def get_edge_indices(
        pixel_row, neg_sample=DEFAULT_NEG_SAMPLE, threshold=DEFAULT_THRESH):
    """Finds the edges of the subject in a row of pixels. This assumes that
    the subject is on a background with significantly different RGB values.
    Returns a `row_data` instance."""
